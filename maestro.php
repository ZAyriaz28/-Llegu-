...
// 1. SEGURIDAD: Verificar que sea maestro
if (!isset($_SESSION['rol']) || $_SESSION['rol'] !== 'maestro') {
    header("Location: index.html");
    exit;
}

// 2. OBTENER DATOS REALES PARA LAS TARJETAS

// A. Contar Total de Estudiantes (Rol ID 3 = Estudiante)
$stmt = $db->prepare("SELECT COUNT(*) FROM usuarios WHERE rol_id = 3");
$stmt->execute();
$totalAlumnos = $stmt->fetchColumn();

// B. Contar Asistencias de Hoy
$fechaHoy = date('Y-m-d');
$stmt = $db->prepare("SELECT COUNT(*) FROM asistencias WHERE fecha = ?");
$stmt->execute([$fechaHoy]);
$asistenciasHoy = $stmt->fetchColumn();

// C. Calcular Porcentaje de Asistencia de Hoy
$porcentajeHoy = 0;
if ($totalAlumnos > 0) {
    $porcentajeHoy = round(($asistenciasHoy / $totalAlumnos) * 100);
}

// D. Detectar Alumnos en Riesgo (Ejemplo: Más de 3 faltas)
// (Esta consulta asume que tienes lógica de faltas, por ahora pondremos 0 o un conteo real si tienes la tabla)
$alertaDesercion = 0; // Aquí iría una consulta SQL más compleja
